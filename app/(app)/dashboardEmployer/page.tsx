...
export default function DashboardEmployerPage() {
  ...
  // Redirect non-admins away as soon as the user record loads
  useEffect(() => {
  ...
  const dailySummary = useQuery(
    ...
    employerId ? { employerId, date: today } : "skip"
  );

  const createStaffMutation = useMutation(api.staff.createStaffFromDashboard);
  ...
  const [selectedForDelete, setSelectedForDelete] = useState<{ _id: string; firstName: string; lastName: string } | null>(null);

  const [form, setForm] = useState({ firstName: "", lastName: "", email: "", role: "", department: "" });
  const [inviteEmail, setInviteEmail] = useState("");

  const staffData = (staffList ?? []).map((x: any) => {
    const date = x.lastEntryTime ? new Date(x.lastEntryTime) : null;
    return {
      ...x,
      _id: x.userId,
      lastEntryTime: date
        ? date.toLocaleString("en-US", { year: "numeric", month: "long", day: "numeric", hour: "numeric", minute: "numeric", hour12: true })
        : "N/A",
    };
  });

  const departmentItems = (departments ?? []).map((d: any) => ({ name: d.name, id: d._id }));

  const itemsGenerator = (row: Record<string, unknown>) => [
    [{ label: "View Employee", icon: "i-heroicons-eye-20-solid", click: () => router.push(`/dashboardStaff/${row._id}`) }],
    [{ label: "Delete Employee", icon: "i-heroicons-trash-20-solid", click: () => { setSelectedForDelete(row as any); setShowDeleteModal(true); } }],
  ];
  ...
  const handleInviteOne = async () => {
    ...
    setLoading(true);
    try {
      const result = await inviteByEmail({ email: inviteEmail.trim() });
      if (result.success) {
  ...
  const handleInviteAll = async () => {
    ...
    try {
      ...
      if (result.invited > 0) {
        toast(`Sent ${result.invited} invitation${result.invited !== 1 ? "s" : ""}`, "success");
      }
      if (result.failed > 0) {
        toast(`${result.failed} invite${result.failed !== 1 ? "s" : ""} failed`, "error");
      }
    ...
    <main className="min-h-full bg-white p-4 md:p-10">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div className="flex flex-col mb-4 md:mb-0">
          ...
          <span className="text-sm font-light">See Today&apos;s attendance overview..</span>
        </div>
        <div className="flex space-x-2">
          ...
          >
            <svg width="20" height="20" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M13 22.5H6.59087C5.04549 22.5 3.81631 21.748 2.71266 20.6966C0.453366 18.5441 4.1628 16.824 5.57757 15.9816C7.97679 14.553 10.8425 14.1575 13.5 14.7952" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M16.5 7C16.5 9.48528 14.4853 11.5 12 11.5C9.51472 11.5 7.5 9.48528 7.5 7C7.5 4.51472 9.51472 2.5 12 2.5C14.4853 2.5 16.5 4.51472 16.5 7Z" stroke="white" strokeWidth="2"/>
              <path d="M19 23.7426V19.5M19 19.5V15.2574M19 19.5H14.7574M19 19.5H23.2427" stroke="white" strokeWidth="2" strokeLinecap="round"/>
            </svg>
      ...
      <div className="mt-8 md:mt-20">
        <XSummary
          ...
          absentToday={Math.max(0, staffData.length - (dailySummary?.uniqueStaffSignedIn ?? 0))}
        />
        <XTable columns={columns} tableData={staffData} paginationData={paginationData} itemsGenerator={itemsGenerator} />
      </div>
      ...
      >
        <div className="space-y-5 w-full">
          ...
          <FormField label="Role" name="role">
            <Input placeholder="Role" size="lg" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} />
          </FormField>
      ...
      <XModal
        ...
        footer={
          <div className="flex justify-end gap-3">
            <Button color="gray" variant="soft" onClick={() => setShowInviteModal(false)}>Close</Button>
          </div>
        }
      >
        <div className="space-y-6">
          <p className="text-gray-600 text-sm">
            Invite staff to create their Clerk account. They will receive an email to set their password.
          </p>
          ...
          {pendingInviteCount > 0 && (
            <div className="border-t pt-6">
              <p className="text-sm text-gray-600 mb-3">
                {pendingInviteCount} staff member{pendingInviteCount !== 1 ? "s" : ""} haven&apos;t been invited yet.
              </p>
              <Button loading={loading} variant="outline" onClick={handleInviteAll} className="w-full">
